      ...
      while (rollCallsProcessed < maxVotes) {
        ...
        for (const vote of voteList) {
          ...
          try {
            ...
            // FIX: Congress.gov member votes API returns houseRollCallVoteMemberVotes.results
            // (not houseRollCallVote.members.member as previously assumed)
            const raw =
            ...
            for (const member of members) {
              // FIX: API returns bioguideID (capital ID) and voteCast (not votePosition)
              const bioguideId = member.bioguideID ?? member.bioguideId ?? member.bioGuideId;
              const position = normalizePosition(member.voteCast ?? member.votePosition ?? member.memberVote ?? member.vote ?? '');
