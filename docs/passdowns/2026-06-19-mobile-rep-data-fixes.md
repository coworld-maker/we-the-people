...
### Addendum (later 2026-06-19): FEC wired up + OpenStates confirmed
- **OpenStates state bills: CONFIRMED LIVE.** Verified `/states/ca` in the authenticated browser — "Recent state bills" renders fresh bills (SCR 173, SB 1014, AB 2056…, all ~1d old). `OPENSTATES_API_KEY` is set in Vercel; the connection is healthy. (Committees + votes-tracked fallback also confirmed rendering on rep tiles.)
- **FEC donor data wired up.** Verified the `schedules/schedule_a/by_employer` path returns real data end-to-end (tested live w/ DEMO_KEY against Tim Scott's committee). Fixed `lib/api/fec.ts` to drop occupation-status buckets (`RETIRED`/`SELF-EMPLOYED`/`NULL`/etc. via `NON_EMPLOYER_BUCKETS`) that were burying real funders, and bumped `per_page` to 30 so 8 real employers survive the filter. Now surfaces Blackstone/Wells Fargo/Coinbase-type orgs. `LobbyingPanel` already renders donors + an "Add OPEN_FEC_API_KEY" empty state. **The only remaining step is setting `OPEN_FEC_API_KEY` in Vercel** (free key from https://api.open.fec.gov/developers/ — the code intentionally gates on the real key rather than the rate-limited DEMO_KEY). Commit `dae062e`, pushed.
